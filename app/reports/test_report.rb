class TestReport < Prawn::Document
  ...
  def all
    ...
    # Create a signature box with origin at [0, cursor - sigpad_height]
    bounding_box([0, cursor], width: sigpad_width, height: sigpad_height) do
      stroke_bounds
      @sig.each do |e|
        # Because the axis direction is different between pdf and canvas
        # We need to flip the signature over the x-axis then
        # translate the signature up by sigpad_height
        # if the dimension of the signature box in Prawn is different
        # from the signature box in SignaturePad, you will need to add
        # some offset to make sure it looks centered
        stroke { line [x_offset + e["lx"], y_offset + sigpad_height - e["ly"]],
                      [x_offset + e["mx"], y_offset + sigpad_height - e["my"] ] }
      end
    end
